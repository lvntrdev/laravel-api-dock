...
- **`release.sh` refuses a tag whose version has no CHANGELOG entry.** The release gate now also checks that `## [X.Y.Z]` exists for the version being tagged and that nothing is left under `## Unreleased`, so a tag can no longer ship changes that were never written down. `--no-verify` skips it with the rest of the quality gate.
