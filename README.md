...
- **Try it, through your server** — the reader sends the request they are looking at, and the package proxies
  ...
  your own application's host only until you widen the allowlist. Session scoping (logout deletes stored
  credentials) can be traded for cross-session persistence via `try_it.profile_persistence.enabled` — see
...
- Credentials are stored server-side, encrypted with the application encrypter, and are not kept in the browser.
  By default they last exactly as long as the reader's login: logging out takes them with it. That guarantee can
  be traded for cross-session persistence via `try_it.profile_persistence.enabled` — see
  [Configuration reference](#configuration-reference) and [Try-it security contract](#try-it-security-contract)
...
| `try_it.profile_persistence.enabled` | `false` | Moves credential profiles from the session into the cache, keyed by the authenticated user id, so they survive logout — and are shared live across every concurrent session of that user (a second browser or device logged in as the same user sees and can use the same profile list, not just a later re-login). Off by default: the session-scoped guarantee (logout deletes credentials, one browser's profiles are its own) holds for every consumer unless this is explicitly turned on. Profiles are still encrypted before they reach the cache, but anyone who can log back in as that user gets them back. A guest request (no authenticated user) always falls back to session storage regardless of this setting. |
| `try_it.profile_persistence.ttl_minutes` | `43200` (30 days) | How long a persisted profile survives in the cache before it expires. Only takes effect when `profile_persistence.enabled` is true. Requires a cache store that is actually shared and persistent (`redis`, `memcached`, `database`); with `array` the profiles die with the process, and with a per-node `file` store they follow whichever node served the write — both undermine the point of turning this on. |
| `try_it.profile_persistence.key_namespace` | `null` | Optional closure adding a namespace segment to the cache key on top of the auth guard. The guard alone is enough to keep two guards or providers from colliding on the same numeric id, but not a tenant-per-database app whose per-tenant `users` table restarts ids at 1 while every tenant shares one cache store — set this to `fn () => tenant('id')` (or similar) in that case. |
